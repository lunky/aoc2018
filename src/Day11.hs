...
sumPowerLevels serial list = sum $ map (getPowerLevel serial)  list

getGrid :: [(Int,Int)]
getGrid =  [ (x,y) | x <- [1..300], y <- [1..300]] 

sumTbT :: Int -> (Int, Int) -> Int
sumTbT serial start = sum $ map (getPowerLevel serial)  $ fst $ xByX (start,3)
...
squaresXByX (x,y) = takeWhile (\((_,_),a) -> (a+x <= bound+1) && (a+y <= bound+1)) $ map ((x,y),) [1..]
    where bound = 300
