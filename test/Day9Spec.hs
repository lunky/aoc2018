...
spec = do
    describe "Day9 turn" $ do
        it "should give the next board state" $ do
            let input = (2,2,0,[0,2,1])
            let expected = (3,3,0,[0,2,1,3])
            turn input `shouldBe` expected
        it "should give the next board state" $ do
            let input = (3,3,0,[0,2,1,3])
            let expected = (4,4,0,[0,4,2,1,3])
            turn input `shouldBe` expected
        it "should give the next board state" $ do
            let input = (0,0,0,[0])
            let expected = (19,19, 0,[0,16,8,17,4,18,9,19,2,10,5,11,1,12,6,13,3,14,7,15])
            (iterate turn input)!!19 `shouldBe` expected
        it "different behaviour on multiples of 23" $ do
            let input = (0,0,0,[0])
            let expected = (23, 19, 32,[0,16,8,17,4,18,19,2,20,10,21,5,22,11,1,12,6,13,3,14,7,15])
